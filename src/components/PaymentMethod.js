...
export const PaymentMethod = ({ fundAmount }) => {
  ...
  useEffect(() => {
    if (formSubmitted) {
      ...
      if (!cardNumber) {
        setCCNumberError("Please enter the credit card number.");
      ...
      if (!expDate) {
        setCCExpiryError("Please enter the card expiry.");
      } else if (expDate.length < 4) {
        setCCExpiryError(
          "Card expiry is too short. Must be in MMYY format."
        );
      } else {
        setCCExpiryError("");
      }
